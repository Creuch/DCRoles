...
public class Command extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        if(userData.get("exists").equalsIgnoreCase("false")) { event.reply(TextHandling.replaceDiscordPlaceholders(p.getName(), event.getMember(), langConfig.getString("discord.invalidUsername"))).setEphemeral(true).queue();
            return; }
        if(!userData.get("code").equalsIgnoreCase(code)) { event.reply(TextHandling.replaceDiscordPlaceholders(p.getName(), event.getMember(), langConfig.getString("discord.invalidCode"))).setEphemeral(true).queue();
            return; }
        if(userData.get("used").equalsIgnoreCase("true")) { event.reply(TextHandling.replaceDiscordPlaceholders(p.getName(), event.getMember(), langConfig.getString("discord.codeUsed"))).setEphemeral(true).queue();
            return; }
        if(userData.get("role").equalsIgnoreCase("default")) { event.reply(TextHandling.replaceDiscordPlaceholders(p.getName(), event.getMember(), langConfig.getString("discord.defaultRole"))).setEphemeral(true).queue();
            return; }
        ...
        for(String id : discordIDs) {
            ...
            if(role == null) { Bukkit.getConsoleSender().sendMessage(TextHandling.getFormatted("{P}&c Invalid role ID\n&8» &7RoleName: " + userData.get("role") + "\n&8» &7ID: " + id));
            } else {
